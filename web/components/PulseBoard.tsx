...
export default function PulseBoard({ items, categories }: { items: PulseItem[]; categories: string[] }) {
  ...
  // ⚡ Bolt Optimization: Memoize the filtered list to prevent unnecessary
  // re-calculations when other state (like email input) changes.
  const filtered = useMemo(
    () => (selected === 'All' ? items : items.filter((i) => i.category === selected)),
    [items, selected]
  )

  // ⚡ Bolt Optimization: Memoize the toggle function to maintain referential stability
