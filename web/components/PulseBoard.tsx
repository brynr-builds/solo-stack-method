...
export default function PulseBoard({ items, categories }: { items: PulseItem[]; categories: string[] }) {
  ...
  const [submitted, setSubmitted] = useState(false)
    ...
    // ⚡ Bolt: Memoize filtered list to prevent O(N) recalculation on every render (e.g. keystrokes)
  // Expected impact: Reduces main thread blocking during fast state updates (email input)
  const filtered = useMemo(() => selected === 'All' ? items : items.filter((i) => i.category === selected), [selected, items])
