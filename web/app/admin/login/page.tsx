        ...
        <div className="bg-white rounded-xl shadow-lg p-8">
          {!useBackup && !useSetupSecret ? (
            <PasskeyForm
              email={email}
              isDev={isDev}
              onUseBackup={() => setUseBackup(true)}
              onUseSetupSecret={() => setUseSetupSecret(true)}
            />
          ) : useSetupSecret ? (
            <SetupSecretForm
              email={email}
              onBack={() => setUseSetupSecret(false)}
            />
          ) : (
            <BackupForm
              email={email}
              isDev={isDev}
              onBack={() => setUseBackup(false)}
              onUseSetupSecret={() => {
                setUseBackup(false)
                setUseSetupSecret(true)
              }}
            />
          )}
        </div>
