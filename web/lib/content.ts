...
function readType(type: ContentType): Article[] {
  ...
  return fs
    ...
    .map((f) => {
      ...
      return parseArticle(type, f.replace(/\.md$/, ''), raw)
    })
...
export function getArticle(type: ContentType, slug: string): Article | undefined {
  // OPTIMIZATION: Retrieve single item by directly reading the specific file using a sanitized slug
  // Avoids O(N) disk I/O and CPU overhead from reading and parsing the entire directory
