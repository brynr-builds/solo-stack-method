...
function parseArticleFile(type: ContentType, slug: string, raw: string): Article {
...
function readType(type: ContentType): Article[] {
  ...
  return fs
    ...
    .map((f) => {
      ...
      return parseArticleFile(type, f.replace(/\.md$/, ''), raw)
    })
...
export function getArticle(type: ContentType, slug: string): Article | undefined {
  // OPTIMIZATION: Read only the requested file instead of parsing the entire directory
  // Expected impact: O(1) file read/parse instead of O(N) where N is all articles
  ...
  return parseArticleFile(type, safeSlug, raw)
}
