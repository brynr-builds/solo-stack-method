...
function parseArticleFile(type: ContentType, filename: string, raw: string): Article {
...
function readType(type: ContentType): Article[] {
  ...
  return fs
    ...
    .map((f) => {
      ...
      return parseArticleFile(type, f, raw)
    })
...
export function getArticle(type: ContentType, slug: string): Article | undefined {
  ...
  // Instead of reading and parsing ALL markdown files in the directory to find a single match (O(N)),
  // directly read the specific file by slug to avoid O(N) disk I/O and CPU bottlenecks (parsing marked).
  ...
  return parseArticleFile(type, `${safeSlug}.md`, raw)
}
