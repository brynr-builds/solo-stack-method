...
function parseRawArticle(type: ContentType, filename: string, raw: string): Article {
...
function readType(type: ContentType): Article[] {
  ...
  return fs
    ...
    .map((f) => {
      ...
      return parseRawArticle(type, f, raw)
    })
...
export function getArticle(type: ContentType, slug: string): Article | undefined {
  // ⚡ Bolt: O(1) direct file retrieval bypasses O(N) directory reads and parsing
  ...
  try {
    ...
    return parseRawArticle(type, `${safeSlug}.md`, raw)
  } catch (e) {
    return undefined
  }
}
