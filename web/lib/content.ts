...
// ⚡ Bolt: Extract article parsing logic to avoid duplication
...
export function getArticle(type: ContentType, slug: string): Article | undefined {
  // ⚡ Bolt: Optimize O(N) directory scan to O(1) direct file read
  // Use path.basename(slug) to prevent path traversal
