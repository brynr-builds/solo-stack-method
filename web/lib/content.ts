...
function parseArticle(type: ContentType, filePath: string, slug: string): Article {
...
function readType(type: ContentType): Article[] {
  ...
  return fs
    ...
    .map((f) => parseArticle(type, path.join(dir, f), f.replace(/\.md$/, '')))
...
export function getArticle(type: ContentType, slug: string): Article | undefined {
  ...
  // Optimization: read just the single requested file instead of parsing all files in the directory
  return parseArticle(type, filePath, safeSlug)
}
