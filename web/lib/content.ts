...
function parseArticleFile(type: ContentType, filename: string): Article | undefined {
...
function readType(type: ContentType): Article[] {
  ...
  return fs
    ...
    .map((f) => parseArticleFile(type, f) as Article)
...
export function getArticle(type: ContentType, slug: string): Article | undefined {
  ...
  return parseArticleFile(type, `${safeSlug}.md`)
}
