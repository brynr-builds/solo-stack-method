...
function parseFile(type: ContentType, dir: string, filename: string): Article {
...
export function getArticle(type: ContentType, slug: string): Article | undefined {
  // ⚡ Bolt: Replace O(N) directory scan + parse with O(1) direct file read
  // Prevents disk I/O and CPU bottleneck by only parsing the requested markdown file
  const dir = path.join(CONTENT_ROOT, type)
  const safeFilename = `${path.basename(slug)}.md`
  const filePath = path.join(dir, safeFilename)

  if (!fs.existsSync(filePath)) return undefined
  return parseFile(type, dir, safeFilename)
}
