...
function parseArticleFile(type: ContentType, filePath: string, filename: string): Article {
...
function readType(type: ContentType): Article[] {
  ...
  return fs
    ...
    .map((f) => parseArticleFile(type, path.join(dir, f), f))
...
export function getArticle(type: ContentType, slug: string): Article | undefined {
  ...
  return parseArticleFile(type, filePath, `${safeSlug}.md`);
}
