...
// Shared helper to parse a single article to keep data structures consistent.
...
export function getArticle(type: ContentType, slug: string): Article | undefined {
  ...
  // Read specific file directly in O(1) instead of reading and parsing the entire directory (O(N)).
  const sanitizedSlug = path.basename(slug)
  const filePath = path.join(CONTENT_ROOT, type, `${sanitizedSlug}.md`)
  if (!fs.existsSync(filePath)) return undefined
  try {
    const raw = fs.readFileSync(filePath, 'utf8')
    return parseArticleFile(type, `${sanitizedSlug}.md`, raw)
  } catch (e) {
    return undefined
  }
}
