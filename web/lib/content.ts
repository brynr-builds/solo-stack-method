...
function parseArticle(type: ContentType, f: string, dir: string): Article {
...
export function getArticle(type: ContentType, slug: string): Article | undefined {
  // ⚡ Bolt: Optimize getArticle to read single file directly instead of O(N) directory scan
  const dir = path.join(CONTENT_ROOT, type)
  const safeSlug = path.basename(slug) // Prevent path traversal
  const filePath = path.join(dir, `${safeSlug}.md`)

  if (fs.existsSync(filePath)) {
    return parseArticle(type, `${safeSlug}.md`, dir)
  }
